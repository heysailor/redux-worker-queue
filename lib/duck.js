"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
}
Object.defineProperty(exports, "__esModule", { value: true });
const redux_1 = require("redux");
const effects_1 = require("redux-saga/effects");
const lodash_1 = require("lodash");
const sagas_1 = require("./coordinator/sagas");
const duck_1 = __importDefault(require("./queue/duck"));
const duck_2 = __importDefault(require("./flag/duck"));
var ActionTypes;
(function (ActionTypes) {
    ActionTypes["__CLEAR__"] = "__WORKER_QUEUE__CLEAR__";
    ActionTypes["FLUSH"] = "__WORKER_QUEUE__FLUSH";
    ActionTypes["CLEAN"] = "__WORKER_QUEUE__CLEAN";
})(ActionTypes = exports.ActionTypes || (exports.ActionTypes = {}));
// Actions
// Wipe queue  ## DANGER ZONE ##
exports.__clearQueue__ = () => ({
    type: ActionTypes.__CLEAR__,
});
exports.clean = () => ({
    type: ActionTypes.CLEAN,
});
exports.flush = () => ({
    type: ActionTypes.FLUSH,
});
const allReducers = redux_1.combineReducers({
    queue: duck_1.default,
    flag: duck_2.default,
});
const rootReducer = redux_1.combineReducers({
    workerQueue: allReducers,
});
exports.default = rootReducer;
// Sagas
const combinedSagas = Object.assign({}, sagas_1.sagas);
function* rootSagas() {
    yield effects_1.all(lodash_1.map(combinedSagas, effects_1.fork));
}
exports.rootSagas = rootSagas;
//# sourceMappingURL=duck.js.map