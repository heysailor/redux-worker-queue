    ...
    describe('Actions', () => {
        describe('addOrUpdateFlag', () => {
            ...
            test('adds a updatedAt property to the flag on update with existing createdAt field', () => {
                const updatedFlagAction = duck_1.addOrUpdateFlag(queueItem, {
                    status: 'WORKING',
                    handlerIndex: 0,
                ...
                expect(check.test(updatedFlagAction.flag.updatedAt)).not.toEqual(updatedFlagAction.flag.createdAt);
            });
        });
        describe('removeFlag', () => {
            test('it exists', () => {
