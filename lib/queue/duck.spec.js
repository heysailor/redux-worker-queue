    ...
    describe('Actions', () => {
        describe('addOrUpdateItem()', () => {
            ...
            test('it takes an existing queueItem and returns an action with ADD_OR_UPDATE_ITEM actionType and matching item', () => {
                ...
                expect(action).toBeDefined();
                expect(Object.keys(action)).toMatchObject(['type', 'item']);
                expect(action.type).toEqual(queueDuck.QueueActionTypes.ADD_OR_UPDATE_ITEM);
                expect(Object.keys(action.item)).toMatchObject(Object.keys(Object.assign({}, existingItem, { updatedAt: new Date().toJSON() })));
            });
            describe('removeItem()', () => {
