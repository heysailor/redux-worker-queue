...
class WorkerQueue {
    ...
    addOrUpdateQueueItem(item) {
        ...
        store_1.store.dispatch(this.actions.addOrUpdateItemAndClean(Object.assign({ clientMutationId }, item)));
    ...
    set lockoutPeriod(periodInIntegerMs) {
        if (periodInIntegerMs !== 0 &&
            (!lodash_1.isInteger(periodInIntegerMs) || periodInIntegerMs < 0))
            return;
